(ns integrant.core
  (:refer-clojure :exclude [ref run!])
  (:require [clojure.walk :as walk]
            [clojure.set :as set]
            [clojure.string :as str]
            [crinklywrappr.dependency :as dep]))
...
(defn- build-key [f resolvef system [k v]]
...
(defn build
  ...
  ([config keys f]
   (build config keys f (fn [_ v] v)))
  ([config keys f resolvef]
   {:pre [(map? config)]}
   (let [relevant-keys   (dependent-keys config keys)
     ...
     (reduce (partial build-key f resolvef)
...
(defn expand
  ...
  [config]
  (build config (keys config) (fn [_ v] v) resolve-key))
...
(defn init
  ...
  ([config keys]
   {:pre [(map? config)]}
   (build config keys init-key resolve-key)))
